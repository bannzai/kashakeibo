...
#
# 1. upload keystore と key.properties を git 管理外のディレクトリ (KASHAKEIBO_ANDROID_SECRET_DIR。既定 ~/.config/kashakeibo/android) に生成する。既にあれば再利用する
...
#
# 使い方: bash scripts/android/setup-release-signing.sh [--skip-firebase] [--skip-github] [--gcp-backup]
...
if [ "$SKIP_FIREBASE" -eq 0 ]; then
  ...
  if firebase apps:android:sha:list "$APP_ID" --project "$FIREBASE_PROJECT" --json | jq -e --arg sha "$SHA1" '.result[] | select(.shaHash == $sha)' > /dev/null; then
    echo "SHA-1 は $FIREBASE_PROJECT に登録済みです"
  else
    firebase apps:android:sha:create "$APP_ID" "$SHA1" --project "$FIREBASE_PROJECT" > /dev/null
    echo "SHA-1 を $FIREBASE_PROJECT の Android アプリ ($APP_ID) に登録しました"
  fi
...
if [ "$GCP_BACKUP" -eq 1 ]; then
  ...
  backup_to_gcp googleplay-upload-keystore "$KEYSTORE_FILE"
  backup_to_gcp googleplay-upload-keyproperties "$KEY_PROPERTIES_FILE"
fi
