...
- [ ] **Google サインインシートの表示**: Google のバックアップ導線をタップすると Google のサインイン UI が表示される (実アカウントの入力・リンク完了までは行わない)
  - 自動化: manual (同上)
  - ❌ 失敗: debug ビルド (bundle ID `com.bannzai.kashakeibo.dev`) で「Googleでリンク」→「続ける」と進むと、Google のサインイン UI が出ずにスナックバー `PlatformException(google_sign_in, Your app is missing support for the following URL schemes: com.googleusercontent.apps.750726705707-kp1f1kc6qj63l3miu8726rvc88uohop1, NSInvalidArgumentException, null)` が表示される。再現手順: debug ビルドを起動 → 設定 → 「Googleでリンク」→ 確認ダイアログの「続ける」。原因は `ios/Runner/Info.plist` の `CFBundleURLTypes` に共有 Extension 用スキームしか無く、`ios/Firebase/dev/GoogleService-Info.plist` の `REVERSED_CLIENT_ID` (上記の `com.googleusercontent.apps....`) が URL スキームとして登録されていないこと。`ios/Firebase/prod/GoogleService-Info.plist` には `REVERSED_CLIENT_ID` 自体が存在しないため、release ビルドでも同様に Google リンクが動かない可能性がある。issue: #66 (修正 PR: #70)
  - 追記 (2026-08-23): 真因は Info.plist の記述漏れではなく、`Copy GoogleService-Info.plist` ビルドフェーズの位置だった。REVERSED_CLIENT_ID を URL スキームへ追記する処理は元から入っていたが、フェーズが Xcode の Info.plist 生成より前にあり、追記が生成結果で上書きされていた。prod 側も OAuth クライアント未構成ではなく plist が古かっただけ。PR #70 のブランチで再実行して Google のサインイン UI が出ることを確認済み (エビデンスは PR #70 の QA.md)。**この項目は PR #70 がマージされた後に ✅ へ更新する** (このブランチ単体では修正が入っていないため未チェックのままにしている)
...
- [x] **アカウント削除の確認ダイアログ**: 「アカウントを削除」をタップすると確認ダイアログ (「アカウントを削除しますか？」・完全に削除され元に戻せない旨) が表示され、キャンセルすると何も起きない
  - 自動化: manual (Maestro 未導入のため agent のシミュレータ操作で確認する)
- [x] **アカウント削除の実行**: 確認ダイアログで「削除する」を実行すると、明細データが消えて初期状態 (新しい匿名ユーザー・明細 0 件) になる
