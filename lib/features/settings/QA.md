...
- [ ] **OSS ライセンスの表示**: 法務リンク欄の「ライセンス」からパッケージ一覧を開き、ライセンス本文を確認して設定画面へ戻れる
  - 自動化: manual (シミュレータで一覧・本文・戻る操作を確認する。今回の追加項目は未検証)
...
シミュレータでの画面確認は未実施 (リソース占有のため)。2026-09-09 のユーザー指示により画面確認を省略し、静的解析とテストを今回の検証とした。`flutter analyze` は指摘なし（exit 0）、`flutter test --no-pub --concurrency=1` は191件成功・1件スキップ（exit 0）。スキップは既存の App Store アセット生成用テストで、追加したライセンス一覧・本文・戻る操作のテストは成功した。コード生成・iOS／Android ビルドは未実施。実描画を確認していないため、チェックと最終画面確認日は更新していない。
