...
採用・破棄を選ぶ。実際の撮影で背景 (机・手等) が映り込んでも紙面・画面だけから抽出し、
1 枚の画像に別々のレシートが複数写っていればレシートごとに 1 明細、1 回の支払いが複数の紙片に
分かれて写っていれば (長いレシート・分割発行の領収書) 1 明細にまとめる (issue #82。
抽出ルールの SSOT は workers/image/README.md の POST /analyses)。共有 Extension から受け取った画像も同じ確認画面へ合流する (`features/share_import`)。
