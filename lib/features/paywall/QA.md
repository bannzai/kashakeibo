...
| S4 | プレミアム判定は entitlement `premium` で行い、購入後は残量チップが「スキャン無制限」、設定のプラン行が「プレミアム」、ペイウォールは「プレミアム利用中」になる | 購入後の残量チップ / 購入後の設定のプラン行 / プレミアム利用中の表示 |
...
- [x] **購入後の残量チップ**: 購入直後に月次一覧の残量チップが「スキャン無制限」になる
  - 自動化: manual (Maestro 未導入のため agent のシミュレータ操作で確認する)
