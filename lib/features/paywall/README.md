...
スキャン無料枠 (月 10 回) を超えたユーザーにプレミアム (スキャン無制限 + 全期間の履歴) の購入を促すハードペイウォール (issue #12)。
...
- `PaywallPage` (`showPaywall` で全画面ダイアログとして開く。閉じた時の値はプレミアムになったら true)
  - ✕ (閉じる) → 72px のスパークル円 → 見出し「スキャン、し放題に。」
  - 無料枠バー「今月の無料スキャン n/10」(無料プランのみ。`monthlyScanQuotaProvider`)
  - 特典 3 点 (スキャン無制限 / 全期間の履歴 / 今後の新機能)
  ...
  - 自動更新の説明と利用規約・プライバシーポリシーへのリンク (サブスクリプションの表示要件)
...
1. 導線 (残量チップ / 記録するシートで残量 0 の「カメラで撮影」/ 設定のプラン行 / 撮影フローで解析が 402) からペイウォールを開く。`showPaywall` の `trigger` で導線を Analytics に記録する
