...
- 実際の撮影を想定した抽出ルール (issue #82。プロンプトは `src/analysis.ts`): 背景 (机・手・財布等) の映り込みは無視する。1 枚の画像に別々の支払いのレシートが複数写っていればレシートごとに 1 件。1 回の支払いが複数の紙片に分かれて写っている場合 (長いレシート・分割発行の領収書) は、同一支払いの連続した断片だと明確に判断できる時 (品目の行が途中から続く・合計や決済の記載が全体で 1 つだけ等) に限りまとめて 1 件にし、それぞれに合計がある完結したレシートは同じ店名・日付でも別々の 1 件にする。品質は合成フィクスチャ (`scripts/generate-analysis-fixtures.py` の `receipt_two_receipts.jpg` / `receipt_split_long.jpg` / `receipt_same_store_two_receipts.jpg`) と実物ベンチマーク (`benchmark/`) で検証する
