...
export interface ImageAnalysisResult {
   ...
   * 1 枚の画像に別々の支払いのレシートが複数写っていればそれぞれ 1 件、1 回の支払いが
   * 複数の紙片に分かれて写っていれば (長いレシート・分割発行の領収書) まとめて 1 件 (issue #82)。
   */
...
const analysisPrompt = [
  ...
  "- 実際の撮影では机・手・財布などの背景や周囲の物が一緒に写り込みます。背景は無視し、レシート・明細の紙面や画面の内容だけから抽出します",
  "- 紙のレシートは 1 枚につき 1 明細とし、amount はレシートの支払総額 (税込の合計) を使います。商品行ごとに分割しません",
  "- 1 枚の画像に別々の支払いのレシートが複数写っている場合は、レシートごとに 1 明細にします",
  "- 長いレシートや分割発行の領収書が複数の紙片に分かれて写っている場合は、同一支払いの連続した断片だと明確に判断できる時 (品目の行が途中から続いている、合計・決済の記載が全体で 1 つだけ等) に限り、全体を 1 つの支払いとして 1 明細にまとめ、amount はその支払全体の総額 (合計の記載があればその値) を使います",
  "- それぞれに合計がある完結したレシートは、同じ店名・同じ日付でも別々の支払いとして 1 明細ずつにします",
  "- ただし、レシートとクレジットカードの利用控え (売上票) のように、同じ 1 回の決済から発行された複数の控えが写っている場合 (店名・日時・金額が一致する等) は、二重計上せず 1 明細にまとめます",
  "- カード明細・購入履歴のスクリーンショットは、取引 1 件ごとに 1 明細にします",
  "- title は店名・サービス名 (摘要)。EC の購入履歴では、サイト名だけでなく注文内容がわかる商品名を摘要に使います。読み取れない場合は空文字にします",
